...
public class UserServiceImpl implements UserService {
    ...
    public void callDefragProc() {
	 ...
	 @Transactional(propagation = Propagation.REQUIRED, isolation = Isolation.READ_COMMITTED)
    @Override
	public String approveUser(UserModel usr, HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        userDao.approveUser(usr);                
        String[] roles = usr.getRoles().split(",");
        List<String> formRoles = Arrays.asList(roles);
        List<String> dbRoles = userDao.getUserRoles(usr.getUserId());
        for (String role : formRoles) {
        ...
        */
        securityDao.insertSysAudit("Approved User " + usr.getUserId(), (String) session.getAttribute("userid"),
                    (String) session.getAttribute("usersi"), (String) session.getAttribute("userip"), "Success");
        return "success";        
	    }
	...
	public GridPage<UserModel> listUnApprovedUsers(int page, int max, String sidx, String sord, String searchField,
			String searchOper, String searchString) {
		 return userDao.listUnApprovedUsers(page, max, sidx, sord, searchField, searchOper, searchString);
	}
}