...
public class AdminController {
    ...
    public String approveUserPage(HttpServletRequest request) {
    ...
    @RequestMapping(value = "/listUnApprovedUsers", method = RequestMethod.GET)
    public @ResponseBody
    GridPage<UserModel> listUnApprovedUsers(
            @RequestParam(value = "page", required = false) int page,
            @RequestParam(value = "max", required = false) int max,
            @RequestParam(value = "sidx", required = false) String sidx,
            @RequestParam(value = "sord", required = false) String sord,
            ...
            @RequestParam(value = "searchOper", required = false) String searchOper,
            @RequestParam(value = "searchString", required = false) String searchString) {
        final GridPage<UserModel> list = userService.listUnApprovedUsers(page, max, sidx, sord, searchField,
                searchOper, searchString);
        return list;
    }
    ...
    public String getUserId(@PathVariable("id") String id, Model model, Map<String, List> map,
            Map<String, String> mapS) {        
        model.addAttribute("userModel", userService.getUserDetails(id));
        map.put("locale", userService.getLocales());
        map.put("roleList", userService.getUserRoles(id));        
    ...
    public @ResponseBody
    MessageModel approveUserPost(@Valid UserModel userModel, BindingResult result,
            HttpServletRequest request) {
        csrfTokenService.removeTokenFromSession(request);
        MessageModel m = new MessageModel();
        if (result.hasErrors()) {
        ...
        String ret = userService.approveUser(userModel, request);
        if (ret.equals("success")) {
            m.setMesgType(ret);
            m.setMesgValue(messageSource.getMessage("admin.user.approve", new Object[]{userModel.getUserName()}, null, null));
            return m;
        } else {
            m.setMesgType(ret);
            m.setMesgValue(messageSource.getMessage("internalError", null, null));
