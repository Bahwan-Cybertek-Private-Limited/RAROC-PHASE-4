...
public class RarocServiceImpl implements RarocService {
    ...
    public RarocMasterModel getFees(String ratingId) {
        return rarocDao.getFees(ratingId);
    }
    ...
    public void addETBfac(RarocMasterModel model, String user) {
        String audseq = securityDao.getAuditSequence();
        securityDao.insertOperAudit(audseq, "Add RAROC Master", user,
                "success", "Ref number '" + model.getRarocref() + "' Added");
        rarocDao.addETBfac(model, user);
    }

    @Override
    public String getCustType(String refId) {
        return rarocDao.getCustType(refId);
    }

    @Override
    public GridWithFooterPage<RarocViewModel> gridRarocNewViewETB(String ref, Integer unit) {
        return rarocDao.gridRarocNewViewETB(ref, unit);
    }

    @Override
    public void submitFacilityETB(String recref, String derivate, HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        String userid = (String) session.getAttribute("userid");
        String audseq = securityDao.getAuditSequence();
        securityDao.insertOperAudit(audseq, "Submit RAROC", userid,
                "success", "Current FY RAROC submitted for reference id '" + recref);
        System.out.println("I ahere oo");
        rarocDao.submitRarocDetailsETB(recref, derivate);
    }
