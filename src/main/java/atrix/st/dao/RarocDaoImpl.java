...
//        inpt.setFacilities(facs);
 ...
 /*String query = " With tab as (   "
    ...
    @Override
    public RarocMasterModel getFees(String ratingId) {
        String sql = "SELECT COUNT(*) FROM t_rdm_ram_cust_mast Where V_TOOL_CODE = ? ";
        int rowCount = getJdbcTemplate().queryForObject(sql, new Object[]{ratingId}, Integer.class);
        RarocMasterModel obj = null;
        if (rowCount == 0) {
            ...
            String query = " SELECT TRUNC((select NVL(sum(NVL(avg_book_bal,0)),0) avg_book_bal from v_rdm_roc_customer where prod_grp_id=124 and v_tool_code= ?)/100000,2) CA_CDAB "
                    + "       , TRUNC((select NVL(sum(NVL(current_acct_fees,0)),0)  from v_rdm_roc_customer where v_tool_code =  ? )/100000,2) CA_Fee  "
                    + "       , TRUNC((select NVL(sum(NVL(exchange_income,0)),0)  from v_rdm_roc_customer where  v_tool_code= ?)/100000,2) Forex_Spot "
                    + "       , TRUNC((select NVL(sum(NVL(gfid_fees,0))+sum(NVL(service_charges,0))+sum(NVL(tps_comm,0))+sum(NVL(card_charges,0))+sum(NVL(remit_comm,0))+sum(NVL(dir_income,0))+sum(NVL(mb_comm,0))+sum(NVL(misc_income,0)),0) oth from v_rdm_roc_customer where v_tool_code = ?)/100000,2) Others "
                    + "       , TRUNC((select NVL(sum(NVL(cms_fee,0)),0)  from v_rdm_roc_customer where  v_tool_code = ? )/100000,2) CMS_Fee   "
                    + "       , TRUNC((select nvl(sum(nvl(avg_book_bal,0)),0)  from v_rdm_roc_customer where prod_grp_id=128 and v_tool_code= ? )/100000,2) Term_Deposit_CDAB "
                    + " FROM DUAL";
            obj = (RarocMasterModel) getJdbcTemplate().queryForObject(
                    query, new Object[]{ratingId, ratingId, ratingId, ratingId, ratingId, ratingId}, new RowMapper<RarocMasterModel>() {
                @Override
                public RarocMasterModel mapRow(ResultSet rs, int i) throws SQLException {
                    RarocMasterModel obj = new RarocMasterModel();
                    obj.setBbcdab(rs.getString("CA_CDAB"));
                    obj.setCafee(rs.getString("CA_FEE"));
                    obj.setForex(rs.getString("FOREX_SPOT"));
                    obj.setOther(rs.getString("OTHERS"));
                    obj.setCms(rs.getString("CMS_FEE"));
                    obj.setTdfee(rs.getString("TERM_DEPOSIT_CDAB"));
                    return obj;
                }
            });
    ...
    @Override
    public void addETBfac(RarocMasterModel model, String user) {
        model.setUnit(100000);
        System.out.println(model.getRid() + " Ref No. = " + model.getRarocref());
        String query = " INSERT INTO INPT_RAROC(N_FACILITY_NO,v_rec_ref_no,V_FAC_DESC,V_FAC_TYPE,D_ORIGIN_DATE,V_SHORT_EXT,N_AMOUNT,N_ORIGINAL_MATURITY,v_curr,N_INT_RATE_COMM,N_COST_FUNDS,N_UPFRONT_FEE,v_facility_type) "
                + " SELECT MAX(ROWNUM) ,'" + model.getRarocref() + "', account_number,CASE WHEN PROD_GROUP = 'Cash Credit' THEN 'CC' WHEN PROD_GROUP = 'Overdrafts' THEN 'OD' ELSE PROD_GROUP END PROD_GROUP, "
                + " ORIGINATION_DATE,  UPPER(external_rating) ,"
                + " TRUNC(SUM(NVL(avg_book_bal,0)),2) avg_book_bal ,"
                + " round((maturity_date-ORIGINATION_DATE)/30) ||'M' CONTRACTUAL_MATURITY, "
                + " 'INR' CURRENCY,"
                + " TRUNC(AVG(INTEREST_RATE_PERC)/100,4) INT,"
                + " TRUNC(AVG(NET_FTP_PERC)/100,4) FTP, "
                + " TRUNC(ROUND(SUM(NVL(PROCESSING_FEES,0))),2) COMM, 'FB'   "
                + " FROM v_rdm_roc_customer Where prod_grp_id IN (110,103,102) and capital > 100000 AND V_TOOL_CODE = ? "
                + " GROUP BY account_number,PROD_GROUP,ORIGINATION_DATE, external_rating,maturity_date";

        getJdbcTemplate().update(query, new Object[]{
            model.getRid()
        });

        String query2 = " INSERT INTO INPT_RAROC(N_FACILITY_NO,v_rec_ref_no,V_FAC_DESC,V_FAC_TYPE,D_ORIGIN_DATE,V_SHORT_EXT,N_AMOUNT,N_ORIGINAL_MATURITY,v_curr,N_INT_RATE_COMM,N_COST_FUNDS,N_UPFRONT_FEE,v_facility_type)  "
                + " SELECT MAX(ROWNUM) ,'" + model.getRarocref() + "', account_number,CASE WHEN PROD_GROUP = 'Bills' THEN 'Other Bills' ELSE PROD_GROUP END PROD_GROUP,ORIGINATION_DATE, external_rating ,SUM(NVL(avg_book_bal,0)) avg_book_bal ,"
                + " round((maturity_date-ORIGINATION_DATE)/30) ||'M' CONTRACTUAL_MATURITY,  "
                + " 'INR' CURRENCY,"
                + " TRUNC(AVG(INTEREST_RATE_PERC)/100,4) INT, "
                + " TRUNC(AVG(NET_FTP_PERC)/100,4) FTP, "
                + " TRUNC(ROUND(SUM(NVL(PROCESSING_FEES,0)+ NVL(BILL_LC_BG_COMM,0))),2) COMM, 'FB' "
                + " FROM v_rdm_roc_customer Where prod_grp_id IN (101) AND V_TOOL_CODE = ? "
                + " GROUP BY account_number,PROD_GROUP,ORIGINATION_DATE, external_rating, maturity_date ";

        getJdbcTemplate().update(query2, new Object[]{
            model.getRid()
        });

        String sql = " INSERT INTO INPT_RAROC(N_FACILITY_NO, v_rec_ref_no,V_FAC_DESC,V_FAC_TYPE,D_ORIGIN_DATE,V_SHORT_EXT,N_AMOUNT,N_ORIGINAL_MATURITY,v_curr,N_INT_RATE_COMM,N_COST_FUNDS,N_UPFRONT_FEE,v_facility_type)"
                + " SELECT MAX(ROWNUM) ,'" + model.getRarocref() + "', account_number,"
                + " CASE WHEN PROD_GROUP = 'BG' THEN 'Financial BG / SBLC / Buyers Credit' WHEN PROD_GROUP = 'LCs' THEN 'LC' ELSE PROD_GROUP END PROD_GROUP,ORIGINATION_DATE, external_rating ,SUM(NVL(avg_book_bal,0)) avg_book_bal ,"
                + " round((maturity_date-ORIGINATION_DATE)/30) ||'M' CONTRACTUAL_MATURITY, "
                + " 'INR' CURRENCY,"
                + " TRUNC(AVG(INTEREST_RATE_PERC)/100,4) INT, "
                + " TRUNC(AVG(NET_FTP_PERC)/100,4) FTP, "
                + " TRUNC(ROUND(SUM(PROCESSING_FEES+ BG_COMMISSION+LC_COMMISSION)),2) COMM, 'NFB' "
                + " FROM v_rdm_roc_customer Where prod_grp_id IN (104,105) AND V_TOOL_CODE = ? "
                + " GROUP BY account_number,PROD_GROUP,ORIGINATION_DATE, external_rating,maturity_date ";
        ...
        String updateSql = "UPDATE INPT_RAROC SET  N_BA_CDAB = ?, N_TERM_DEPO = ?, N_CA_FEE = ?, N_CMS_FEE = ?, N_OTHER = ?,N_FOREX_COMMIS = ?, V_DATA_POP='P', N_FACILITY_NO = ROWNUM  WHERE v_rec_ref_no = ?";
    ...
    @Override
    public String getCustType(String ref) {
        String query = "SELECT V_CUST_TYPE FROM inpt_raroc_master  WHERE V_REC_REF_NO = ? ";
        return getJdbcTemplate().queryForObject(query, new Object[]{ref}, String.class);
    }

    @Override
    public GridWithFooterPage<RarocViewModel> gridRarocNewViewETB(String ref, Integer unit) {
        List<RarocViewModel> facs = new ArrayList<>();
        RarocViewModel footer = new RarocViewModel();
        String sql = "Select ((nvl(N_PRO_AXIS_FUND,0)* nvl(N_NPLL,0)) * 0.75)/100000 val FROM INPT_RAROC_MASTER Where v_rec_ref_no = ?";
        Double ln_protion = getJdbcTemplate().queryForObject(sql, new Object[]{ref}, Double.class);
        String query = "WITH TAB AS ( "
                + "SELECT  N_FACILITY_NO, V_FACILITY_DESC, V_FACILITY_TYPE, D_DISBURS_DATE, V_EXTERNAL_RAT, V_EXPOSURE_GAURNTY,  "
                + "V_PSL_FLAG, N_RWA_INTENSITY, V_CURR, V_CONTRA_MATURITY, N_AVG_MAT, N_AVG_UTILIZATION, V_INT_TYPE, V_INT_BENCHMARK,  "
                + "N_INT_RATE, N_BASE_FTP_PER, N_PSL_CHARGE, N_LIQ_TENOUR_PER, N_BASIC_RISK, N_NET_FTP_PER, N_NOTION_RET_PER, N_NIM, "
                + "N_STD_EAD, N_IRB_EAD, N_AVG_OS_BAL, N_CAPITAL, N_LIB_CDAB, N_INT_INCOME, N_BASE_FTP_AMT, N_PSL_CHARGE_AMT, N_LIQ_PRI_AMT,  "
                + "N_NET_FTP_AMT, N_NOTION_RET_AMT, N_NII_AMT, N_COMMISION_FEE, N_NON_CRE_FEE, N_OPEX, N_EL, N_PBT, N_PAT, N_ANNUALIZED_PAT,  "
                + "N_RWA, N_RAROC "
                + "FROM rslt_raroc_raroc_new WHERE V_REC_REF_NO = ? ORDER BY N_FACILITY_NO ASC) "
                + "SELECT * FROM TAB "
                + "UNION "
                + "Select 100 n_facility_no, null v_fac_desc,null v_fac_type, null D_ORIGIN_DATE,null V_LONG_EXT, null N_EXP_GUAR, null V_PSL, "
                + "SUM(N_RWA_INTENSITY),MAX(v_curr),null n_original_maturity,null n_tenure,null N_AVG_UTL_CUR,null v_int_type,null v_benchmark, "
                + "AVG(N_INT_RATE),AVG(N_BASE_FTP_PER),NULL,NULL,NULL,NULL,NULL, SUM(n_nim),SUM(N_STD_EAD),SUM(N_IRB_EAD), null N_AVG_BAL_CUR,SUM(N_CAPITAL), "
                + "SUM(N_LIB_CDAB),SUM(N_INT_INCOME),SUM(N_BASE_FTP_AMT),NULL,NULL,NULL,NULL,NULL, SUM(N_NII_AMT),SUM(N_COMMISION_FEE),SUM(N_NON_CRE_FEE),SUM(n_opex), "
                + "SUM(n_el),SUM(n_pbt),SUM(n_pat), SUM(N_RWA) + 1 ,AVG(N_RAROC) END  "
                + "from tab "
                + "UNION "
                + "Select 101 n_facility_no, null v_fac_desc,null v_fac_type, null D_ORIGIN_DATE,null V_LONG_EXT, null N_EXP_GUAR, null V_PSL, "
                + "SUM(N_RWA_INTENSITY),MAX(v_curr),null n_original_maturity,null n_tenure,null N_AVG_UTL_CUR,null v_int_type,null v_benchmark, "
                + "AVG(N_INT_RATE),AVG(N_BASE_FTP_PER),NULL,NULL,NULL,AVG(N_NET_FTP_PER),NULL, SUM(n_nim),SUM(N_STD_EAD),SUM(N_IRB_EAD), null N_AVG_BAL_CUR,SUM(N_CAPITAL), "
                + "SUM(N_LIB_CDAB),SUM(N_INT_INCOME),SUM(N_BASE_FTP_AMT),SUM(N_BASE_FTP_AMT),NULL,NULL,NULL,NULL, SUM(N_NII_AMT),SUM(N_COMMISION_FEE),SUM(N_NON_CRE_FEE),SUM(n_opex), "
                + "SUM(n_el),SUM(n_pbt),SUM(n_pat), SUM(N_RWA), "
                + "AVG(N_RAROC) END  "
                + "from tab";
        ...
        String[] othTotal = new String[cols];
        String[] total = new String[cols];
        String[] relationTotal = new String[cols];
        outputName[0] = "Facility Description";
        outputName[1] = "Facility Type";
        outputName[2] = "Date of Disbursement";
        outputName[3] = "External Rating (Borrower/Guarantor)";
        ...
        outputName[12] = "Interest rate Benchmark";
        outputName[13] = "Interest Rate(%)";
        outputName[14] = "Base FTP (%)";
        outputName[15] = "PSL (Charge/Incentive) (%)";
        outputName[16] = "Liquidity Tenor Premium (%)";
        outputName[17] = "Basis Risk";
        outputName[18] = "Net FTP (%)";
        outputName[19] = "Notional Return (%)";
        outputName[20] = "NIM (%)";
        outputName[21] = "STD EAD (Rs Lakh)";
        outputName[22] = "IRB EAD (Rs Lakh)";
        outputName[23] = "Avg O/S/ Bal (Rs Lakh)";
        outputName[24] = "Capital ( Rs Lakh)";
        outputName[25] = "Liability CDAB (Rs Lakh)";
        outputName[26] = "Interest Income (Rs Lakh)";
        outputName[27] = "(-) Base FTP (Rs Lakh)";
        outputName[28] = "(-)/(+) PSL Charge / Incentive (Rs Lakh)";
        outputName[29] = "Liquidity Premium (Rs Lakh)";
        outputName[30] = "Basis Risk";
        outputName[31] = "Net FTP (Rs Lakh)";
        outputName[32] = "(+) Notional Return (Rs Lakh)";
        outputName[33] = "NII (Rs Lakh)";
        outputName[34] = "(+)Credit Fee/Commision (Rs Lakh)";
        outputName[35] = "(+)Non-credit Fees (Rs Lakh)";
        outputName[36] = "(-)Opex (Rs Lakh)";
        outputName[37] = "(-)EL (Rs Lakh)";
        outputName[38] = "PBT (Rs Lakh)";
        outputName[39] = "PAT (Rs Lakh)";
        outputName[40] = "RWA (Rs Lakh)";
        outputName[41] = "RAROC (%)";
        for (Map row : rows) {
            if (row.get("n_facility_no") == null) {
                total[0] = fmt.ToString(row.get("v_fac_desc"));
                total[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                total[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                total[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                total[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                total[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                total[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                total[7] = fmt.ToString(row.get("v_curr"));
                total[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                total[9] = fmt.ToString(row.get("N_AVG_MAT"));
                total[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                total[11] = fmt.ToString(row.get("V_INT_TYPE"));
                total[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                total[13] = fmt.ToString(row.get("N_INT_RATE"));
                total[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                total[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                total[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                total[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                total[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                total[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                total[20] = fmt.ToString(row.get("N_NIM"));
                total[21] = fmt.ToString(row.get("N_STD_EAD"));
                total[22] = fmt.ToString(row.get("N_IRB_EAD"));
                total[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                total[24] = fmt.ToString(row.get("N_CAPITAL"));
                total[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                total[26] = fmt.ToString(row.get("N_INT_INCOME"));
                total[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                total[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                total[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                total[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                total[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                total[32] = fmt.ToString(row.get("N_NII_AMT"));
                total[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                total[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                total[35] = fmt.ToString(row.get("N_OPEX"));
                total[36] = fmt.ToString(row.get("N_EL"));
                total[37] = fmt.ToString(row.get("N_PBT"));
                total[38] = fmt.ToString(row.get("N_PAT"));
                total[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                total[40] = fmt.ToString(row.get("N_RWA"));
                total[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 100) {
                creditTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                creditTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                creditTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                creditTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                creditTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                creditTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                creditTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                creditTotal[7] = fmt.ToString(row.get("v_curr"));
                creditTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                creditTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                creditTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                creditTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                creditTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                creditTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                creditTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                creditTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                creditTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                creditTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                creditTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                creditTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                creditTotal[20] = fmt.ToString(row.get("N_NIM"));
                creditTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                creditTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                creditTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                creditTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                creditTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                creditTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                creditTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                creditTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                creditTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                creditTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                creditTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                creditTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                creditTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                creditTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                creditTotal[35] = fmt.ToString(row.get("N_OPEX"));
                creditTotal[36] = fmt.ToString(row.get("N_EL"));
                creditTotal[37] = fmt.ToString(row.get("N_PBT"));
                creditTotal[38] = fmt.ToString(row.get("N_PAT"));
                creditTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                creditTotal[40] = fmt.ToString(row.get("N_RWA"));
                creditTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 101) {
                relationTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                relationTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                relationTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                relationTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                relationTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                relationTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                relationTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                relationTotal[7] = fmt.ToString(row.get("v_curr"));
                relationTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                relationTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                relationTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                relationTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                relationTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                relationTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                relationTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                relationTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                relationTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                relationTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                relationTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                relationTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                relationTotal[20] = fmt.ToString(row.get("N_NIM"));
                relationTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                relationTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                relationTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                relationTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                relationTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                relationTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                relationTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                relationTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                relationTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                relationTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                relationTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                relationTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                relationTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                relationTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                relationTotal[35] = fmt.ToString(row.get("N_OPEX"));
                relationTotal[36] = fmt.ToString(row.get("N_EL"));
                relationTotal[37] = fmt.ToString(row.get("N_PBT"));
                relationTotal[38] = fmt.ToString(row.get("N_PAT"));
                relationTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                relationTotal[40] = fmt.ToString(row.get("N_RWA"));
                relationTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 93) {
                caTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                caTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                caTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                caTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                caTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                caTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                caTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                caTotal[7] = fmt.ToString(row.get("v_curr"));
                caTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                caTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                caTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                caTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                caTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                caTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                caTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                caTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                caTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                caTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                caTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                caTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                caTotal[20] = fmt.ToString(row.get("N_NIM"));
                caTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                caTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                caTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                caTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                caTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                caTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                caTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                caTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                caTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                caTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                caTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                caTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                caTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                caTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                caTotal[35] = fmt.ToString(row.get("N_OPEX"));
                caTotal[36] = fmt.ToString(row.get("N_EL"));
                caTotal[37] = fmt.ToString(row.get("N_PBT"));
                caTotal[38] = fmt.ToString(row.get("N_PAT"));
                caTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                caTotal[40] = fmt.ToString(row.get("N_RWA"));
                caTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 94) {
                saTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                saTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                saTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                saTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                saTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                saTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                saTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                saTotal[7] = fmt.ToString(row.get("v_curr"));
                saTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                saTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                saTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                saTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                saTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                saTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                saTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                saTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                saTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                saTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                saTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                saTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                saTotal[20] = fmt.ToString(row.get("N_NIM"));
                saTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                saTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                saTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                saTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                saTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                saTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                saTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                saTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                saTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                saTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                saTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                saTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                saTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                saTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                saTotal[35] = fmt.ToString(row.get("N_OPEX"));
                saTotal[36] = fmt.ToString(row.get("N_EL"));
                saTotal[37] = fmt.ToString(row.get("N_PBT"));
                saTotal[38] = fmt.ToString(row.get("N_PAT"));
                saTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                saTotal[40] = fmt.ToString(row.get("N_RWA"));
                saTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 95) {
                tdTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                tdTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                tdTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                tdTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                tdTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                tdTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                tdTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                tdTotal[7] = fmt.ToString(row.get("v_curr"));
                tdTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                tdTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                tdTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                tdTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                tdTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                tdTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                tdTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                tdTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                tdTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                tdTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                tdTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                tdTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                tdTotal[20] = fmt.ToString(row.get("N_NIM"));
                tdTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                tdTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                tdTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                tdTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                tdTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                tdTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                tdTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                tdTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                tdTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                tdTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                tdTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                tdTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                tdTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                tdTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                tdTotal[35] = fmt.ToString(row.get("N_OPEX"));
                tdTotal[36] = fmt.ToString(row.get("N_EL"));
                tdTotal[37] = fmt.ToString(row.get("N_PBT"));
                tdTotal[38] = fmt.ToString(row.get("N_PAT"));
                tdTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                tdTotal[40] = fmt.ToString(row.get("N_RWA"));
                tdTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 96) {
                cmsTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                cmsTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                cmsTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                cmsTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                cmsTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                cmsTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                cmsTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                cmsTotal[7] = fmt.ToString(row.get("v_curr"));
                cmsTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                cmsTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                cmsTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                cmsTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                cmsTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                cmsTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                cmsTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                cmsTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                cmsTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                cmsTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                cmsTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                cmsTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                cmsTotal[20] = fmt.ToString(row.get("N_NIM"));
                cmsTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                cmsTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                cmsTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                cmsTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                cmsTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                cmsTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                cmsTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                cmsTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                cmsTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                cmsTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                cmsTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                cmsTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                cmsTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                cmsTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                cmsTotal[35] = fmt.ToString(row.get("N_OPEX"));
                cmsTotal[36] = fmt.ToString(row.get("N_EL"));
                cmsTotal[37] = fmt.ToString(row.get("N_PBT"));
                cmsTotal[38] = fmt.ToString(row.get("N_PAT"));
                cmsTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                cmsTotal[40] = fmt.ToString(row.get("N_RWA"));
                cmsTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 97) {
                fxTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                fxTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                fxTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                fxTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                fxTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                fxTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                fxTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                fxTotal[7] = fmt.ToString(row.get("v_curr"));
                fxTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                fxTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                fxTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                fxTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                fxTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                fxTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                fxTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                fxTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                fxTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                fxTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                fxTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                fxTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                fxTotal[20] = fmt.ToString(row.get("N_NIM"));
                fxTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                fxTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                fxTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                fxTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                fxTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                fxTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                fxTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                fxTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                fxTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                fxTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                fxTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                fxTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                fxTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                fxTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                fxTotal[35] = fmt.ToString(row.get("N_OPEX"));
                fxTotal[36] = fmt.ToString(row.get("N_EL"));
                fxTotal[37] = fmt.ToString(row.get("N_PBT"));
                fxTotal[38] = fmt.ToString(row.get("N_PAT"));
                fxTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                fxTotal[40] = fmt.ToString(row.get("N_RWA"));
                fxTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else if (fmt.ToInteger(row.get("n_facility_no")) == 98) {
                othTotal[0] = fmt.ToString(row.get("v_fac_desc"));
                othTotal[1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                othTotal[2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                othTotal[3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                othTotal[4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                othTotal[5] = fmt.ToString(row.get("V_PSL_FLAG"));
                othTotal[6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                othTotal[7] = fmt.ToString(row.get("v_curr"));
                othTotal[8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                othTotal[9] = fmt.ToString(row.get("N_AVG_MAT"));
                othTotal[10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                othTotal[11] = fmt.ToString(row.get("V_INT_TYPE"));
                othTotal[12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                othTotal[13] = fmt.ToString(row.get("N_INT_RATE"));
                othTotal[14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                othTotal[15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                othTotal[16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                othTotal[17] = fmt.ToString(row.get("N_BASIC_RISK"));
                othTotal[18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                othTotal[19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                othTotal[20] = fmt.ToString(row.get("N_NIM"));
                othTotal[21] = fmt.ToString(row.get("N_STD_EAD"));
                othTotal[22] = fmt.ToString(row.get("N_IRB_EAD"));
                othTotal[23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                othTotal[24] = fmt.ToString(row.get("N_CAPITAL"));
                othTotal[25] = fmt.ToString(row.get("N_LIB_CDAB"));
                othTotal[26] = fmt.ToString(row.get("N_INT_INCOME"));
                othTotal[27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                othTotal[28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                othTotal[29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                othTotal[30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                othTotal[31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                othTotal[32] = fmt.ToString(row.get("N_NII_AMT"));
                othTotal[33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                othTotal[34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                othTotal[35] = fmt.ToString(row.get("N_OPEX"));
                othTotal[36] = fmt.ToString(row.get("N_EL"));
                othTotal[37] = fmt.ToString(row.get("N_PBT"));
                othTotal[38] = fmt.ToString(row.get("N_PAT"));
                othTotal[39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                othTotal[40] = fmt.ToString(row.get("N_RWA"));
                othTotal[41] = fmt.ToString(row.get("N_RAROC"));

            } else {
                resultArray[i][0] = fmt.ToString(row.get("v_fac_desc"));
                resultArray[i][1] = fmt.ToString(row.get("V_FACILITY_TYPE"));
                resultArray[i][2] = fmt.ToString(row.get("D_DISBURS_DATE"));
                resultArray[i][3] = fmt.ToString(row.get("V_EXTERNAL_RAT"));
                resultArray[i][4] = fmt.ToString(row.get("V_EXPOSURE_GAURNTY"));
                resultArray[i][5] = fmt.ToString(row.get("V_PSL_FLAG"));
                resultArray[i][6] = fmt.ToString(row.get("N_RWA_INTENSITY"));
                resultArray[i][7] = fmt.ToString(row.get("v_curr"));
                resultArray[i][8] = fmt.ToString(row.get("V_CONTRA_MATURITY"));
                resultArray[i][9] = fmt.ToString(row.get("N_AVG_MAT"));
                resultArray[i][10] = fmt.ToString(row.get("N_AVG_UTILIZATION"));
                resultArray[i][11] = fmt.ToString(row.get("V_INT_TYPE"));
                resultArray[i][12] = fmt.ToString(row.get("V_INT_BENCHMARK"));
                resultArray[i][13] = fmt.ToString(row.get("N_INT_RATE"));
                resultArray[i][14] = fmt.ToString(row.get("N_BASE_FTP_PER"));
                resultArray[i][15] = fmt.ToString(row.get("N_PSL_CHARGE"));
                resultArray[i][16] = fmt.ToString(row.get("N_LIQ_TENOUR_PER"));
                resultArray[i][17] = fmt.ToString(row.get("N_BASIC_RISK"));
                resultArray[i][18] = fmt.ToString(row.get("N_NET_FTP_PER"));
                resultArray[i][19] = fmt.ToString(row.get("N_NOTION_RET_PER"));
                resultArray[i][20] = fmt.ToString(row.get("N_NIM"));
                resultArray[i][21] = fmt.ToString(row.get("N_STD_EAD"));
                resultArray[i][22] = fmt.ToString(row.get("N_IRB_EAD"));
                resultArray[i][23] = fmt.ToString(row.get("N_AVG_OS_BAL"));
                resultArray[i][24] = fmt.ToString(row.get("N_CAPITAL"));
                resultArray[i][25] = fmt.ToString(row.get("N_LIB_CDAB"));
                resultArray[i][26] = fmt.ToString(row.get("N_INT_INCOME"));
                resultArray[i][27] = fmt.ToString(row.get("N_BASE_FTP_AMT"));
                resultArray[i][28] = fmt.ToString(row.get("N_PSL_CHARGE_AMT"));
                resultArray[i][29] = fmt.ToString(row.get("N_LIQ_PRI_AMT"));//rwa_rs/ead_rs*100
                resultArray[i][30] = fmt.ToString(row.get("N_NET_FTP_AMT"));
                resultArray[i][31] = fmt.ToString(row.get("N_NOTION_RET_AMT"));
                resultArray[i][32] = fmt.ToString(row.get("N_NII_AMT"));
                resultArray[i][33] = fmt.ToString(row.get("N_COMMISION_FEE"));
                resultArray[i][34] = fmt.ToString(row.get("N_NON_CRE_FEE"));
                resultArray[i][35] = fmt.ToString(row.get("N_OPEX"));
                resultArray[i][36] = fmt.ToString(row.get("N_EL"));
                resultArray[i][37] = fmt.ToString(row.get("N_PBT"));
                resultArray[i][38] = fmt.ToString(row.get("N_PAT"));
                resultArray[i][39] = fmt.ToString(row.get("N_ANNUALIZED_PAT"));
                resultArray[i][40] = fmt.ToString(row.get("N_RWA"));
                resultArray[i][41] = fmt.ToString(row.get("N_RAROC"));
        ...
        while (i < cols) {
            if (i == cols - 1) {
                ...
                footer.setOther(othTotal[i]);
                footer.setTotal(creditTotal[i]);
                footer.setRelationTotal(relationTotal[i]);
            } else {
                RarocViewModel obj = new RarocViewModel();
                obj.setId(i + 1);
                obj.setOutputName(outputName[i]);
                ...
                obj.setOther(othTotal[i]);
                obj.setTotal(creditTotal[i]);
                obj.setRelationTotal(relationTotal[i]);
                facs.add(obj);
            }
            i++;
        }
    ...
    @Override
    public void submitRarocDetailsETB(String refRec, String facType) {

        String procName = "";
        String totProcName = "";

        procName = "RAROC_ADD_ETB";
        totProcName = "RAROC_ADD_TOTAL_ETB";

        // Delete the existing data from RSLT_RAROC table for ref id
        String query = "DELETE FROM rslt_raroc_raroc_new WHERE v_rec_ref_no = ?";
        getJdbcTemplate().update(query, new Object[]{refRec});
        Map<String, Object> map = new HashMap<>();
        List<String> lst = listFacNumber(refRec);
        ...
        SimpleJdbcCall jdbcCalls = new SimpleJdbcCall(getJdbcTemplate().getDataSource())
                .withSchemaName(schemaName)
                .withProcedureName(totProcName)
                .withoutProcedureColumnMetaDataAccess()
                .declareParameters(new SqlParameter("refrec", java.sql.Types.VARCHAR));
