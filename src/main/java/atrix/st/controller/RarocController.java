...
//    }
    ...
    public String createNewPost(@ModelAttribute("formRarocMaster") RarocMasterModel model,
        ...
        if (existence.equals("empty")) {
            ...
            map.put("custType", model.getCustType());
...
//                map.put("guarantee", "0");
//            }
            if (model.getCustType().equals("ETB")) {
                rarocService.addETBfac(model, "Amol");
                System.out.println("I am a ETB customer");
            } else {
                System.out.println("I am a NTB customer");
            }
...
//    }
    ...
    //Get the Data on rating id from RAM Database
    @RequestMapping(value = "/data/ratingFees", method = RequestMethod.GET)
    public @ResponseBody
    RarocMasterModel getRatingFees(@RequestParam(value = "rid") String rid) {
        return rarocService.getFees(rid);
    }

    //RAROC View Tab1e ETB
    @RequestMapping(value = "/view/grid/etb", method = RequestMethod.GET)
    public @ResponseBody
    GridWithFooterPage<RarocViewModel> gridRarocViewETB(@RequestParam(value = "ref") String ref,
            HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        Integer unit = (Integer) session.getAttribute("unit");
        return rarocService.gridRarocNewViewETB(ref, unit);
    }

    //View RAROC page
    @RequestMapping(value = "/etb/view", method = RequestMethod.GET)
    public String viewPageETB(Map<String, Object> model, @RequestParam(value = "ref") String ref,
            @RequestParam(value = "viewType", required = false) String viewType, HttpServletRequest request) {
        model.put("form", rarocService.gridRarocHeader(ref));
        if (viewType == null) {
            return "raroc/rarocView";
            ...
            Integer unit = (Integer) session.getAttribute("unit");
            //model.put("grid", rarocService.gridRarocView(ref, unit));
            model.put("grid", rarocService.gridRarocNewViewETB(ref, unit));
            return "XlsRaroc";
        } else {
            HttpSession session = request.getSession(false);
            Integer unit = (Integer) session.getAttribute("unit");
